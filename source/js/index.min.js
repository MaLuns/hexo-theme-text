...
}}if(scrollTop>500){$("#back-top").addClass("show");}else{$("#back-top").removeClass("show");}if(window.CONFIG.is_toc){app.toc();}},toc:function toc(){var viewHeight=window.innerHeight||document.documentElement.clientHeight;var post=document.getElementsByClassName("post-content")[0];if(post){var titles=post.querySelectorAll('h1,h2,h3,h4,h5,h6');var scrollY=window.scrollY;for(var index=0;index<titles.length;index++){var element=titles[index];if(element.offsetTop>=scrollY&&element.offsetTop<=scrollY+viewHeight){if(index>0)element=titles[index-1];$("#post-toc .current").removeClass("current");$('a[href="#'+element.id+'"]').addClass('current');break;}}}}};$(window).scroll(app.scroll);app.initSetting();app.initSettingEvent();app.scroll();mediumZoom('article img',{margin:24,background:'#00000077',scrollOffset:0});});
...
if(result.success){param=_objectSpread(_objectSpread({},parms),result.data);if(!this.atComment){con=this.shadowRoot.querySelector(".list-content");length=con.querySelectorAll('.item-top').length;con.insertBefore(Object(_libs_util__WEBPACK_IMPORTED_MODULE_2__["createList"])([param]),con.children[length]);this._commentList.splice(length,0,param);}else{_con=this.shadowRoot.querySelector("#quote"+this.atComment.topID);param._idxpath=this.atComment.topID+','+_con.children.length;_con.appendChild(Object(_libs_util__WEBPACK_IMPORTED_MODULE_2__["createList"])([param],this.atComment.topID));_com=this._commentList.find(function(item){return item.id===_this3.atComment.topID;});if(_com){_com.childer=_com.childer||[];_com.childer.push(param);}this._cancel_reply();}this.textarea_hidden.innerHTML='';this.textarea.value='';this.textarea.style.height='40px';}this.sending=false;case 10:case"end":return _context3.stop();}}},_callee3,this);}));function _send(_x){return _send2.apply(this,arguments);}return _send;}()},{key:"_cancel_reply",value:function _cancel_reply(){this.shadowRoot.querySelector(".edit-container").appendChild(this.shadowRoot.getElementById('c-comment'));this.atComment=null;}}]);return ComComment;}(/*#__PURE__*/_wrapNativeSuper(HTMLElement));!customElements.get('com-comment')&&customElements.define('com-comment',ComComment);
